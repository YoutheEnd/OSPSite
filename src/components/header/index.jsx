...
class Header extends React.Component {
    ...
    render() {
        ...
        let nav = [];
        siteConfig[language].pageMenu.forEach((item) => {
            if (item.list.length == 0) {
                let str = <li className={classnames({
                    'menu-item': true,
                    [`menu-item-${type}`]: true,
                ...
                {item.list.forEach((btn) => {
                    menuItem.push(<Menu.Item key={btn.text}>
                                    <a target="_blank" rel="noopener noreferrer" href={getLink(btn.link)}>
                                        {btn.text}
                                    </a>
                                </Menu.Item>)
                })};
                let menu = <Menu><Menu.ItemGroup children={menuItem}></Menu.ItemGroup></Menu>;
                let str = <Dropdown
                    overlay={menu}
